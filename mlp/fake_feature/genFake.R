...
set.seed(1234)
# For testing code related to model training
...
write.csv(dt[1:10000, ], file = 'feature.csv', row.names=F)

# For testing code related to 'load_model()'
dt <- data.table(matrix(nrow=n_samp, ncol = 12))
colnames(dt) <- c('sampleID', paste0('a_f', 1:5), paste0('b_f', 1:5), 'win')
...
write.csv(dt[1:n_samp, ], file = 'input_with_wincol.csv', row.names=F)
write.csv(dt[1:n_samp, -ncol(dt), with=F],
          file = 'input_no_wincol.csv', row.names=F)
